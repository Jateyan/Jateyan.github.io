...
微前端是一种将前端巨石应用分解成更小、更简单的子应用的架构风格。它将微服务的理念应用于浏览器端，即将 Web 应用`由单一的单体应用转变为多个小型前端应用聚合为一的应用`。各个前端应用还可以`独立运行、独立开发、独立部署`。解决了下面一系列问题：
...
## 14. 为什么qiankun是单实例框架，单实例控制的原理

qiankun实现单实例控制的核心原理包括：

**基于路由的应用注册与匹配**
- 子应用通过activeRule(激活规则)注册到主框架
- 当URL变化时，框架根据当前路由匹配对应的子应用
- 路由匹配规则决定哪个应用应该被激活

**生命周期管理机制**
- 加载前卸载当前应用：在加载新应用前，会先触发当前应用的unmount生命周期
- 严格的状态转换：应用状态在NOT_LOADED → LOADING → MOUNTED之间严格转换
- 单一渲染容器：默认配置下，所有子应用共享同一个渲染容器，物理上限制了多应用并存
**应用实例控制**
```javascript
 // 简化的核心逻辑
   async function loadApp(app) {
     // 1. 卸载当前正在运行的应用
     if (currentApp) {
       await unmountApplication(currentApp);
     }
     
     // 2. 加载新应用资源
     const { bootstrap, mount, unmount } = await loadAppAssets(app);
     
     // 3. 初始化应用
     await bootstrap();
     
     // 4. 挂载应用到容器
     await mount({ container: '#subapp-container' });
...
```
**沙箱隔离**
- JS沙箱：使用Proxy技术创建JS隔离环境，确保全局变量不冲突
- CSS隔离：通过Shadow DOM或动态样式前缀，防止样式污染
- 沙箱设计为应用激活时创建，失活时销毁，强化了单实例特性

这种设计使得qiankun天然支持单实例模式，简化了应用管理逻辑，减少了内存占用和潜在冲突，适合SPA应用改造和大型应用拆分的微前端场景。
