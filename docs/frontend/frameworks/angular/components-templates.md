...
Angular组件从创建到销毁的整个过程中，会经历一系列生命周期钩子函数，这些钩子函数允许我们在特定时刻执行自定义逻辑。

### 生命周期钩子概述

生命周期钩子是Angular框架在组件生命周期的关键时刻调用的接口方法。每个钩子都有特定用途和执行时机。
...
1. **constructor** - 组件实例创建时
2. **ngOnChanges** - 输入属性变化时(如果有绑定输入属性)
3. **ngOnInit** - 组件初始化，首次ngOnChanges之后
4. **ngDoCheck** - 每次变更检测运行时
5. **ngAfterContentInit** - 组件内容初始化后
6. **ngAfterContentChecked** - 组件内容检查后
7. **ngAfterViewInit** - 组件视图初始化后
8. **ngAfterViewChecked** - 组件视图检查后
9. **ngOnDestroy** - 组件销毁前
...
ngOnChanges(changes: SimpleChanges) {
  ...
  if (changes.userInput) {
    console.log('前一个值:', changes.userInput.previousValue);
    console.log('当前值:', changes.userInput.currentValue);
    console.log('首次变化?', changes.userInput.firstChange);
...
**最佳实践**：
- 使用SimpleChanges参数检查哪些属性变化了
- 通过firstChange属性区分首次变化和后续变化
- 避免重复计算，使用previousValue比较
...
ngDoCheck() {
  // 自定义变更检测逻辑
  if (this.previousObjectValue !== JSON.stringify(this.complexObject)) {
    this.previousObjectValue = JSON.stringify(this.complexObject);
    console.log('复杂对象已变化');
...
ngAfterContentInit() {
  // 内容投影初始化后的处理
  console.log('投影内容已初始化');
  console.log('投影的子组件数量:', this.projectedComponents.length);
...
ngAfterContentChecked() {
  // 内容投影检查后的处理
  if (this.previousContentState !== this.currentContentState()) {
    this.previousContentState = this.currentContentState();
...
- 避免复杂操作，会频繁调用
- 使用缓存避免重复执行
- 避免在此修改视图，可能导致ExpressionChangedAfterItHasBeenChecked错误
...
ngAfterViewInit() {
  // 视图初始化后的处理
  console.log('视图已初始化');
  this.chartElement = this.chartElementRef.nativeElement;
...
**最佳实践**：
- 适合集成第三方库（如图表、地图）
- 此时可安全访问ViewChild和ViewChildren引用
- 避免在此直接修改绑定的属性，使用setTimeout解决
...
ngAfterViewChecked() {
  // 视图检查后的处理
  if (this.viewDimensions !== this.getCurrentViewDimensions()) {
    this.viewDimensions = this.getCurrentViewDimensions();
    this.adjustLayout();
  }
}
```
...
export class PerformanceComponent implements OnInit, DoCheck {
  ...
  ngOnInit() {
    // 初始计算一次
    this.cachedResult = this.performHeavyComputation(this.input);
  }
  ...
  private performHeavyComputation(input: string) {
    // 实际的复杂计算
  ...
  ngOnInit() {
    try {
      this.dataService.getData()
        .pipe(
          catchError(err => {
            this.errorService.logError('数据加载失败', err);
            return of({ title: '默认数据' }); // 提供备用数据
          })
        )
        .subscribe(data => {
          this.data = data;
      ...
      this.errorService.logError('初始化错误', err);
    }
  }
}
```
...
export class DynamicLoaderComponent implements OnInit, OnDestroy {
  ...
  loadComponent() {
    ...
    // 设置动态组件的输入
    this.componentRef.instance.data = '动态数据';
    
    // 监听动态组件的输出
    this.componentRef.instance.output.subscribe(data => {
      console.log('动态组件输出:', data);
    });
  }
...
export class FormComponent implements OnInit, OnDestroy, DoCheck {
  ...
  ngDoCheck() {
    // 检测表单状态变化并执行特定操作
    const currentState = JSON.stringify(this.form.getRawValue());
    if (this.previousFormState !== currentState) {
      this.previousFormState = currentState;
...
export class DataTableComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private updateVisibleData() {
    const cacheKey = `${this.currentPage}_${this.pageSize}`;
    
    // 检查缓存
    if (this.cache.has(cacheKey)) {
      this.visibleData = this.cache.get(cacheKey);
      return;
    }
  ...
  private initializeCache() {
    ...
    // 预加载下一页数据
    const nextPageKey = `${this.currentPage + 1}_${this.pageSize}`;
    if (!this.cache.has(nextPageKey)) {
      const start = this.currentPage * this.pageSize;
      const end = start + this.pageSize;
      const nextPageData = this.data.slice(start, end);
      this.cache.set(nextPageKey, nextPageData);
    }
  }
...
Angular组件生命周期钩子是构建响应式、高性能应用的关键工具。合理利用这些钩子可以：
...
遵循生命周期钩子的最佳实践，可以避免常见陷阱，创建更加稳定和高效的Angular应用。
...
2. **样式穿透**：当需要从父组件修改子组件样式时
   - 对于`Emulated`模式，使用`::ng-deep`选择器实现样式穿透：
     ```scss
     /* 基本用法 */
     ...
     /* 推荐与:host结合使用，限制作用范围 */
     :host ::ng-deep .child-component .element {
       color: red;
     }
     ```

   - **`::ng-deep`工作原理**：
     - 在Emulated封装模式下，Angular会给组件内的样式添加特殊属性选择器，限制其只影响组件自身
     - `::ng-deep`会移除这种限制，使选择器可以匹配任何组件中的元素
     - 被`::ng-deep`修饰的选择器会"穿透"组件边界，影响整个应用
   
   - **最佳实践**：
     - 总是与`:host`结合使用，以限制样式穿透的范围
     - 仅用于必要情况，如修改第三方组件样式
     - 避免全局样式污染，谨慎使用过于通用的选择器
     - 为被穿透的样式添加特定的前缀或命名空间
   ...
   - **替代方案**：
     ...
     - 为子组件设计输入属性控制样式
     - 利用组件库提供的主题系统
     - 使用`ViewEncapsulation.None`创建全局样式组件
   
   - **使用案例**：
     ```typescript
     // 修改Angular Material组件样式示例
     @Component({
       selector: 'app-custom-mat-button',
       ...
       styles: [`
         ...
         /* 更好的方式 - 使用特定前缀 */
         :host ::ng-deep .my-app-mat-button.mat-button {
           background-color: #e91e63;
           color: white;
         }
       `]
     })
     export class CustomMatButtonComponent {}
     ```

   - **注意事项**：
     - `:ng-deep`已被弃用，在未来版本可能被完全移除
     - Angular团队推荐使用CSS变量和组件输入属性替代直接样式穿透
     - 过度使用会导致样式难以维护和调试
     - 在大型项目中可能造成意外的样式冲突
